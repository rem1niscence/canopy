...
import {
    clipboard, cpyObj, convertIfTime, isEmpty,
    pagination, upperCaseAndRepUnderscore, withTooltip, convertTx
...
function convertCardData(state, v) {
    ...
    return value.block ? convertBlock(value) : value.validator && !state.modalState.accOnly ? value.validator : value.account
}
...
export default function DetailModal({state, setState}) {
    ...
    if (data === "no result found") {
        return <ToastContainer position={"top-center"} className="search-toast">
    ...
    function renderPageTable(tab) {
        ...
        if ("block" in d) {
            ...
            page = tab === 1 ? convertTransactions(d.sent_transactions.results) : convertTransactions(d.rec_transactions.results)
                ...
                <Table responsive>
                    ...
                    <tr>
                        {Object.keys(convertPaginated(convertTabData(state, data, 1)[0])).map((k, i) => (
                            <td key={i} className="detail-table-row-title">{upperCaseAndRepUnderscore(k)}</td>
                        ))}
                    </tr>
