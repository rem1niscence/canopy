...
import {cpyObj, convertIfTime, convertNumberWCommas, convertTime, isHex, isNumber, pagination, upperCaseAndRepUnderscore, convertTx} from "@/components/util"
...
function convertParams(v) {
    ...
    let value = cpyObj(v)
    return ['Consensus', 'Validator', 'Fee', 'Governance']
        .flatMap(space =>
...
function convertOrder(v) {
    ...
    return {
        ...
        "SellersSellAddress": v.SellersSellAddress,
                ...
                {sortedData.map((val, idx) => (
                    <tr key={idx}>
                        {Object.keys(val).map((k, i) => (
                            <td key={i} className="table-col">{convertValue(k, val[k], props.openModal)}</td>
                        ))}
                    </tr>
                ))}
                </tbody>
