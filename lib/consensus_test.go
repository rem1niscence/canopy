...
func TestAggregateSignatureEquals(t *testing.T) {
	tests := []struct {
		name     string
		detail   string
		...
		{
			name:   "two nil aggregate signatures",
			detail: "any nil signatures automatically returns false",
		},
		{
			name:   "one nil aggregate signature",
			detail: "any nil signatures automatically returns false",
			as1:    nil,
			as2: &AggregateSignature{
		...
		{
			name:   "two different aggregate signatures (sig field)",
			detail: "the signature field is different",
			as1: &AggregateSignature{
				Signature: []byte("signature"),
				Bitmap:    []byte("bitmap"),
			},
			as2: &AggregateSignature{
				Signature: []byte("signature_different"),
				Bitmap:    []byte("bitmap"),
			},
		},
		{
			name:   "two different aggregate signatures (bitmap field)",
			detail: "the bitmap field is different",
			as1: &AggregateSignature{
				Signature: []byte("signature"),
				Bitmap:    []byte("bitmap"),
			},
			as2: &AggregateSignature{
				Signature: []byte("signature"),
				Bitmap:    []byte("bitmap_different"),
			},
		},
		{
			name:   "two equal aggregate signatures",
			detail: "should return true",
			as1: &AggregateSignature{
				Signature: []byte("signature"),
				Bitmap:    []byte("bitmap"),
			},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			require.Equal(t, test.expected, test.as1.Equals(test.as2))
		})
	}
}
...
func TestViewCheck(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			view:      &View{CommitteeId: 1},
			checkView: &View{CommitteeId: 2},
		...
		},
		{
			...
			view:           &View{CanopyHeight: 1},
			checkView:      &View{CanopyHeight: 2},
			enforceHeights: true,
