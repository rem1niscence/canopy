...
func newTxn(t *testing.T, prefix []byte) (*Txn, *badger.DB, *badger.WriteBatch) {
	...
	return NewBadgerTxn(reader, writer, prefix, false, true, version, false), db, writer
}
...
func TestNestedTxn(t *testing.T) {
	...
	nested := NewTxn(baseTxn, baseTxn, []byte("2/"), false, true, baseTxn.writeVersion, false)
...
func TestLiveWriteVsNormalWrite(t *testing.T) {
	...
	// liveWrite enabled
	txn1 := NewBadgerTxn(reader1, writer1, prefix, false, true, version, true)
	// second transaction without live write
	reader2 := db2.NewTransactionAt(version, false)
	writer2 := db2.NewWriteBatchAt(version)
	// liveWrite disabled
	txn2 := NewBadgerTxn(reader2, writer2, prefix, false, true, version, false)
