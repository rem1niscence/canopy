...
/*
	...
	Fallback to Pseudorandom if none found
...
func VerifyCandidate(p *SortitionVerifyParams) (out []byte, isCandidate bool) {
	...
	msg := formatInput(p.LastProposersPublicKeys, p.Height, p.Round)
...
func weightedPseudorandom(p *PseudorandomParams) (publicKey crypto.PublicKeyI) {
	seed := crypto.Hash(formatInput(p.LastProposersPublicKeys, p.Height, p.Round))[:16]
